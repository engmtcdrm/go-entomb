...
// Tests for [PermissionTestReadOnly] function.
func Test_PermissionTestReadOnly(t *testing.T) {
	t.Run("0-arg function", func(t *testing.T) {
		tempDir := t.TempDir()
		tempFile, err := CreateTempFile(tempDir, "test-*.txt")
		require.NoError(t, err)
		...
		err = PermissionTestReadOnly(tempFile.Name(), func() error { return nil })
		require.NoError(t, err)
	})
	...
	t.Run("1-arg function", func(t *testing.T) {
		...
		err = PermissionTestReadOnly(tempFile.Name(), func(arg1 string) error { return nil }, tempFile.Name())
		require.NoError(t, err)
	})
	...
	t.Run("2-arg function", func(t *testing.T) {
		...
		err = PermissionTestReadOnly(tempFile.Name(), func(arg1, arg2 string) error { return nil }, tempFile.Name(), "arg2")
		require.NoError(t, err)
	})
	...
	t.Run("variadic function", func(t *testing.T) {
		...
		err = PermissionTestReadOnly(tempFile.Name(), func(args ...string) error {
			if len(args) != 2 {
				return errors.New("expected 2 arguments")
			}
			return nil
